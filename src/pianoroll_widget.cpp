	...
	: wxWindow(parent, id, pos, size, wxBORDER_SIMPLE, name)
{
	...
	scWindow = new PianorollCanvas(topsplitter, wxID_ANY, wxDefaultPosition, wxDefaultSize, "pianoroll");

	wxWindow *automationTmpWindow = new wxWindow(topsplitter, wxID_ANY);
	automationBox = new wxBoxSizer(wxVERTICAL);
	automationToolBar = new wxToolBar(automationTmpWindow, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxTB_HORIZONTAL, "ccgraphtools");
	automationTestButton = new wxButton(automationToolBar, wxID_ANY, "FOO!", wxDefaultPosition, wxDefaultSize, 0, wxDefaultValidator, "ccgraphtoolbutton");
	scAutomation = new ControlChangeGraph(automationTmpWindow, wxID_ANY, wxDefaultPosition, wxDefaultSize, "ccgraph");
...
ControlChangeGraph::ControlChangeGraph(wxWindow *parent, wxWindowID id, const wxPoint &pos, const wxSize &size, const wxString &name)
	: wxWindow()
{
	//SetBackgroundStyle(wxBG_STYLE_PAINT);
...
void ControlChangeGraph::DrawTimeline(wxDC &dc)
{
    ...
    const double left_margin = pianorollCanvas->pianoroll_leftmargin + pianorollCanvas->white_width;
    const double interval = pianorollCanvas->measure_width / pianorollCanvas->measure_division;
...
void ControlChangeGraph::OnLeftClick(wxMouseEvent &event)
{
	...
	auto point = event.GetLogicalPosition(dc);
	std::cout << "Mouse click: (" << point.x + hScroll << ", " << point.y << ")" << std::endl;
